...
public class UserService {
	...
	public boolean isCoordinator(String token) throws Exception {
		return this.getFirebaseTokenFromIdToken(token).getClaims().get("admin").equals(true);
	}

	public ResponseEntity<JSONObject> getUser(String token) {
		try {
			...
			if (this.isCoordinator(token)) {
				return new ResponseEntity<>((new JSONObject()).put("role", "Coordinator")
						.put("name", firebaseToken.getName())
						.put("email", firebaseToken.getEmail()), HttpStatus.OK);
			}
